...
from utils import plot_loss, plot_roc, save_metrics_csv
...
# 2. 交叉验证划分
def split_data(df):
    train_test = df.sample(frac=1, random_state=SEED)
    split_idx = int(TRAIN_RATIO * len(train_test))
    train_df, test_df = train_test.iloc[:split_idx], train_test.iloc[split_idx:]
...
def run_fold(train_csv, val_csv, fold):
    ...
    plot_loss(train_losses, f"Fold{fold} Train Loss",
              os.path.join(RES_DIR, f"train_loss_fold{fold}.png"))
    plot_loss(val_losses, f"Fold{fold} Val Loss",
              os.path.join(RES_DIR, f"val_loss_fold{fold}.png"))
    return best_auc
